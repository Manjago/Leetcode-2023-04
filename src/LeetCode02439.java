...
public class LeetCode02439 {
    ...
    private int minimizeArrayValue(int[] nums, int leftInc, int rightEx) {
        ...
        switch (n) {
            case 0: throw new IllegalStateException("Bad logic");
            case 1: {
                System.out.println("For single " + nums[leftInc] + " return " + nums[leftInc]);
                return nums[leftInc];
            }
            case 2: {
                int result = smartMin(nums[leftInc], nums[leftInc + 1]);
                System.out.println("For twin " + nums[leftInc] + "," + nums[leftInc + 1] + " return " + result);
                return result;
            }
            default: {
                ...
                int result = hardMin(leftMin, rightMin);
                System.out.println("For hard " + leftMin + "," + rightMin + " return " + result);
                return result;
            }
        }
    }

    static int smartMin(int a, int b) {
        // 10, 1 -> 10
        // 1, 10 -> 5, 6 -> 6
        // 4, 5 -> 5
        ...
        int part1 = delta  / 2;
        int part2 = delta  % 2;
        int realDelta = part1 + part2;
        return a + realDelta;
    }

    static int hardMin(int a, int b) {
        if (a > b) {
            return a;
        } else if (a == b) {
            return a;
